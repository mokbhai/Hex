...
class SettingsManager:
    ...
    async def load(self) -> HexSettings:
        """Load settings from disk.

        Reads the settings JSON file and deserializes it into a HexSettings instance.
        If the file doesn't exist or contains invalid data, returns default settings.

        Returns:
            HexSettings instance with loaded or default values
        ...
        try:
            ...
            # Deserialize into HexSettings
            return HexSettings.from_dict(data)
    ...
    async def save(self, settings: Optional[HexSettings] = None) -> None:
        """Save settings to disk.

        Serializes the provided HexSettings instance to JSON and writes it to disk.
        Creates the configuration directory if it doesn't exist.

        Args:
            settings: HexSettings instance to save. If None, saves default settings.
        ...
        Examples:
            >>> manager = SettingsManager()
            >>> settings = HexSettings(soundEffectsEnabled=False)
            >>> await manager.save(settings)
        """
